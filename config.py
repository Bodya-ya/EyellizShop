...
class Config:
    ...
    STAR_PRICE_BUY: Decimal = Decimal(os.getenv("STAR_PRICE_BUY", "1.2"))  # ₽ за звезду при покупке
    STAR_PRICE_SELL: Decimal = Decimal(os.getenv("STAR_PRICE_SELL", "1.63"))  # ₽ за звезду при продаже
    STAR_MIN_AMOUNT: int = int(os.getenv("STAR_MIN_AMOUNT", "10"))  # Минимум звёзд
    ...
    # Rates (за 1 монету в рублях)
    RATE_BUY: Decimal = Decimal(os.getenv("RATE_BUY", "0.00087"))  # мы покупаем у пользователя
    RATE_SELL: Decimal = Decimal(os.getenv("RATE_SELL", "0.00115"))  # мы продаём пользователю
